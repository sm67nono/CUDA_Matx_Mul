...
__global__ void Dummy_testkernel()
{
	return;
}
...
void getAllDeviceProperties() {
	...
	for (int i = 0; i < nDevices; i++) {
		...
		cout << " Max Grid Size(Blocks per Grid): " <<" X-"<<prop.maxGridSize[0]<<" Y-"<<prop.maxGridSize[1]<<" Z-"<<prop.maxGridSize[2]<< endl;
		cout << " Max Block Size(Threads per Block): " << prop.maxThreadsPerBlock<<endl << endl << endl;
	}
}
...
cudaError_t performMultiGPUJacobi(unsigned int val_dim, unsigned int numJacobiIt, float* val_A0, float* val_A1, float* val_A2, float* val_A3, float* val_A4, float* val_A5, float* val_A6, float* val_rhs, float* val_x_in)
{
	...
	*/
	...
	unsigned int chunk_X = dim / numberOfDevicesAlong_X;
	unsigned int chunk_Y = dim / numberOfDevicesAlong_Y;
	//Added Z for 3D : important-> The chunk size would change for numberOfDevicesAlong_Z: 3D GPU topology
	unsigned int chunk_Z = dim / numberOfDevicesAlong_Z;
	...
	int blocksize = 32;
	//Block Size could be adjusted

	//======================================================Important==============================================================================
	//Note: The division should set a grid size less than or equal to 65535 for sm_20(Fermi) architechture as grid limitation is 65535 x 65535 x 65535
	//Note: The division should set a grid size less than or equal to (2^31-1) along X  for sm_30(Keplar) architechture as grid limitation is (2^31 - 1) x 65535 x 65535
	//=============================================================================================================================================

	dim3 grid(DIVRND((myDim.x * myDim.y * myDim.z), blocksize));
	//dim3 grid((2^31)-1,65535,1);//for sm_30 architechture. Returns Success,but a higher value returns error.
	//int grid = 65536;//for sm_20 architechture. Success, but a higher value returns error
	...
	cout << "Grid Dims " << myDim.x << " X " << myDim.y << " X " << myDim.z << endl;
	cout<<"Block Dim " << blocksize << endl;
	...
	for (int i = 0; i < iterations; i++)
	{
		...
		for (int dev = 0; dev < numDevices; dev++)
		{
			...
			//Dummy_testkernel <<<grid, block, 0, streams[dev] >>> ();
			//cudaDeviceSynchronize();
			/*if (auto err = cudaGetLastError())
			{
				cout << "Kernal Launch Error: " << cudaGetErrorString(err) <<" Iteration "<<i<< endl;
				return err;
			}*/
	...
	if (auto err = cudaGetLastError())
	{
		cout << "Iteration Error: " << cudaGetErrorString(err) << endl;
		return err;
	}
