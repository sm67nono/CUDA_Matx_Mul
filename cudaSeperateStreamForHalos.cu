			...
			if ((!p2penabled)) {
				...
				for (int dev = 0;dev < numDevices;dev++)
				{
					...
					if (deviceArray[dev].nHalo_flag == 1)
					{
						...
						cudaSetDevice(dev);
						cudaMemcpyAsync(d_nhalos[dev], x_buffer[dev], chunk_X * sizeof(float), cudaMemcpyDeviceToDevice, nHaloExchange[dev]);
					...
					if (deviceArray[dev].eHalo_flag == 1) {
						...
						cudaSetDevice(dev);
						cudaMemcpyAsync(d_ehalos[dev], y_buffer[dev], chunk_Y * sizeof(float), cudaMemcpyDeviceToDevice, eHaloExchange[dev]);
