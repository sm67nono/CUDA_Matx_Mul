...
void sendToPrint(vector<float> partial_result,int devicePosition_X, int devicePosition_Y, int numberOfDevicesAlong_X, int chunk_X, int  chunk_Y, int dim, int totalSize, vector<float> &result, int numDevices, int currentIteration, int numberOfTotalIterations) {
	...
	*/
	...
	for (int i = 0; i < iterations; i++)
	{
		...
		for(int dev = 0; dev < numDevices;dev++)
		{
			...
			cudaMemcpy(&partial_resultOnHost[0], d_Vec_Out[dev], domainDivision[dev] * sizeof(float), cudaMemcpyDeviceToHost);

			//Check Results and store it 
			
			sendToPrint(partial_resultOnHost, deviceArray[dev].devicePosition_X, deviceArray[dev].devicePosition_Y, numberOfDevicesAlong_X, chunk_X, chunk_Y, dim, size, result, numDevices, i, iterations);
		

			//Copy the intermediate result from the Host memory to the Device memory

			cudaMemcpy(d_Vec_In[dev], &partial_resultOnHost[0], domainDivision[dev] * sizeof(float), cudaMemcpyHostToDevice);
		...
		//Exchange Halos after each iteration
		exchangehalos_onHost(numDevices, deviceArray, numberOfDevicesAlong_X);
		for (int dev = 0; dev < numDevices; dev++)
		{
