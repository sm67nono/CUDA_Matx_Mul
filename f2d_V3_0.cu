...
void exchangehalos_onHostPinned(int numDevices, vector<create_Device> &deviceArray, int numberofDevicesAlong_X, vector<float*> &nHalosPinned, vector<float*> &sHalosPinned, vector<float*> &eHalosPinned, vector<float*> &wHalosPinned)
{
	//Halos exist in pairs so:
	//Important: A device exchanges North-to-South Pairs and East-to-West Pairs only. Not South-to-North pairs and West-to-East pairs
	//That way the number of exchanges are kept to minimum
...
cudaError_t performMultiGPUJacobi(unsigned int val_dim, unsigned int numJacobiIt, float* val_A0, float* val_A1, float* val_A2, float* val_A3, float* val_A4, float* val_rhs, float* val_x_in)
{
	...
	*/
	...
	for (int dev = 0;dev < numDevices;dev++)
	{
		cudaMallocHost((void**)&nHalo_pinned[dev], (chunk_X * chunk_Y) * sizeof(float));
		cudaMallocHost((void**)&sHalo_pinned[dev], (chunk_X * chunk_Y) * sizeof(float));
		cudaMallocHost((void**)&wHalo_pinned[dev], (chunk_X * chunk_Y) * sizeof(float));
		cudaMallocHost((void**)&eHalo_pinned[dev], (chunk_X * chunk_Y) * sizeof(float));
	...
	for (int i = 0; i < iterations; i++)
	{
		...
		for (int dev = 0; dev < numDevices;dev++)
		{
			...
			//Note: Performance gain on swap vectors on Host rather than using streams and cudaMemcpyAsync
			...
			if (numDevices > 1)
			{
				if (deviceArray[dev].nHalo_flag == 1)
				{
					...
					if (auto err = cudaGetLastError())
					{
						cout << "d_nhalos copy failed 2: " << cudaGetErrorString(err) << endl;
						return err;
					}
				}
				if (deviceArray[dev].sHalo_flag == 1)
				{
					...
					if (auto err = cudaGetLastError())
					{
						cout << "d_shalos copy failed 2: " << cudaGetErrorString(err) << endl;
						return err;
					}
				}
				if (deviceArray[dev].eHalo_flag == 1)
				{
					...
					if (auto err = cudaGetLastError())
					{
						cout << "d_ehalos copy failed 2: " << cudaGetErrorString(err) << endl;
						return err;
					}
				}
				if (deviceArray[dev].wHalo_flag == 1)
				{
					...
					if (auto err = cudaGetLastError())
					{
						cout << "d_whalos copy failed 2: " << cudaGetErrorString(err) << endl;
						return err;
					}
				}
		...
		if (i < iterations) {
			...
			//exchangehalos_onHost(numDevices, deviceArray, numberOfDevicesAlong_X);
			exchangehalos_onHostPinned(numDevices, deviceArray, numberOfDevicesAlong_X,nHalo_pinned, sHalo_pinned, eHalo_pinned,wHalo_pinned);
