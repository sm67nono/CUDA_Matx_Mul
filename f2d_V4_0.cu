			...
			if ((!p2penabled)) {
				...
				for (int dev = 0;dev < numDevices;dev++)
				{
					...
					if (deviceArray[dev].nHalo_flag == 1)
					{
						...
						//Exchange Halos 
						//Send to the device
						cudaMemcpyPeerAsync(x_buffer[dev], dev, d_shalos[devIDtoNorth], devIDtoNorth, chunk_X * sizeof(float), streamsforHaloExcahnge[dev]);
						//Recieve from the device
						cudaMemcpyPeerAsync(d_shalos[devIDtoNorth], devIDtoNorth, d_nhalos[dev], dev, chunk_X * sizeof(float), streamsforHaloExcahnge[dev]);

						cudaMemcpyAsync(d_nhalos[dev], x_buffer[dev], chunk_X * sizeof(float), cudaMemcpyDeviceToDevice, streamsforHaloExcahnge[dev]);
					}
					...
					if (deviceArray[dev].eHalo_flag == 1) {
						...
						//Exchange Halos 
						//Send to the device
						cudaMemcpyPeerAsync(y_buffer[dev], dev, d_whalos[devIDtoEast], devIDtoEast, chunk_Y * sizeof(float), streamsforHaloExcahnge[dev]);
						//Recieve from the device
						cudaMemcpyPeerAsync(d_whalos[devIDtoEast], devIDtoEast, d_ehalos[dev], dev, chunk_Y * sizeof(float), streamsforHaloExcahnge[dev]);

						cudaMemcpyAsync(d_ehalos[dev], y_buffer[dev], chunk_Y * sizeof(float), cudaMemcpyDeviceToDevice, streamsforHaloExcahnge[dev]);
