			...
			if ((!p2penabled)) {
				...
				for (int dev = 0; dev < numDevices; dev++)
				{
					...
					if (deviceArray[dev].nHalo_flag == 1)
					{
						...
						//cudaEventSynchronize(events[dev]);
						...
						//cudaStreamWaitEvent(nHaloExchange[dev], events[dev], 0);
						cudaStreamWaitEvent(nHaloExchange[dev], events[devIDtoNorth], 0);

						cudaMemcpyPeerAsync(x_buffer_north[dev], dev, d_shalos[devIDtoNorth], devIDtoNorth, chunk_X * sizeof(float), nHaloExchange[dev]);
						cudaEventRecord(nHaloEvent[dev], nHaloExchange[dev]);

						//Recieve from the device

						cudaStreamWaitEvent(sHaloExchange[dev], events[dev], 0);
						//cudaStreamWaitEvent(sHaloExchange[dev], events[devIDtoNorth], 0);
						//cudaStreamWaitEvent(sHaloExchange[dev], nHaloEvent[dev], 0);

						cudaMemcpyPeerAsync(x_buffer_south[devIDtoNorth], devIDtoNorth, d_nhalos[dev], dev, chunk_X * sizeof(float), sHaloExchange[dev]);
						...
						cudaStreamWaitEvent(streams[devIDtoNorth], sHaloEvent[dev], 0);
						
						swap(d_nhalos[dev], x_buffer_north[dev]);
						swap(d_shalos[devIDtoNorth], x_buffer_south[devIDtoNorth]);
					...
					if (deviceArray[dev].eHalo_flag == 1) {
						...
						//cudaEventSynchronize(sHaloEvent[dev]);
						//cudaEventSynchronize(nHaloEvent[dev]);
						...
						//Send to the device
						//cudaStreamWaitEvent(eHaloExchange[dev], events[dev], 0);
						cudaStreamWaitEvent(eHaloExchange[dev], events[devIDtoEast], 0);

						cudaMemcpyPeerAsync(y_buffer_east[dev], dev, d_whalos[devIDtoEast], devIDtoEast, chunk_Y * sizeof(float), eHaloExchange[dev]);
						cudaEventRecord(eHaloEvent[dev], eHaloExchange[dev]);

						//Recieve from the device
						
						cudaStreamWaitEvent(wHaloExchange[dev], events[dev], 0);
						//cudaStreamWaitEvent(wHaloExchange[dev], events[devIDtoEast], 0);
						//cudaStreamWaitEvent(wHaloExchange[dev], eHaloEvent[dev], 0);
						
						cudaMemcpyPeerAsync(y_buffer_west[devIDtoEast], devIDtoEast, d_ehalos[dev], dev, chunk_Y * sizeof(float), wHaloExchange[dev]);
						cudaEventRecord(wHaloEvent[dev], wHaloExchange[dev]);
						...
						cudaStreamWaitEvent(streams[devIDtoEast], wHaloEvent[dev], 0);

						swap(d_ehalos[dev], y_buffer_east[dev]);
						swap(d_whalos[devIDtoEast], y_buffer_west[devIDtoEast]);
