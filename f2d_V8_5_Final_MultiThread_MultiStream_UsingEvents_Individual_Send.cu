...
cudaError_t performMultiGPUJacobi(unsigned int val_dim, unsigned int numJacobiIt, float* val_A0, float* val_A1, float* val_A2, float* val_A3, float* val_A4, float* val_rhs, float* val_x_in)
{
	...
	*/
	...
	float *x_buffer_north_ptr_write[4];
	float *x_buffer_south_ptr_write[4];
	float *y_buffer_east_ptr_write[4];
	float *y_buffer_west_ptr_write[4];
			...
			if ((i < (iterations - 1)))
			{
				...
				if ((!p2penabled)) {
					...
					if (deviceArray[dev].nHalo_flag == 1)
					{
						...
						cudaMemcpyPeerAsync(x_buffer_south_ptr_write[devIDtoNorth], devIDtoNorth, nhalo_ptr[dev], dev, chunk_X * sizeof(float), nHaloExchange[dev]);
					...
					if (deviceArray[dev].sHalo_flag == 1)
					{
						...
						int devIDtoSouth = getDeviceIDfromCoord(getDevCoord_X - 1, getDevCoord_Y, numberOfDevicesAlong_X);
						//Exchange Halos 
						...
						cudaMemcpyPeerAsync(x_buffer_north_ptr_write[devIDtoSouth], devIDtoSouth, shalo_ptr[dev], dev, chunk_X * sizeof(float), sHaloExchange[dev]);
					...
					if (deviceArray[dev].eHalo_flag == 1)
					{
						...
						cudaMemcpyPeerAsync(y_buffer_west_ptr_write[devIDtoEast], devIDtoEast, ehalo_ptr[dev], dev, chunk_Y * sizeof(float), eHaloExchange[dev]);
					...
					if (deviceArray[dev].wHalo_flag == 1)
					{
						...
						int devIDtoWest = getDeviceIDfromCoord(getDevCoord_X, getDevCoord_Y - 1, numberOfDevicesAlong_Y);
						//Exchange Halos 
						...
						cudaMemcpyPeerAsync(y_buffer_east_ptr_write[devIDtoWest], devIDtoWest, whalo_ptr[dev], dev, chunk_Y * sizeof(float), wHaloExchange[dev]);
